...
function startupActions() {
    ...
    function scaleMobile(enable = true) {
        if (window.mobileCheck() === enable) {
            docFind("title1").style.transform = "scale(1.25)";
        }
    }
...
function CustomAniFunctions() {
    ...
    function CollapseNavGatesToggle(btnToClick = "#Sdlds", i) {
        let isOn = i;
        ...
        let listTodo = [
            `collap.classList.toggle("expandCollapse1"); // div span
            collap.classList.toggle("expandCollapse2"); // div:nth-last-child(2)
            collap.classList.toggle("expandCollapse3"); //`
        ]
        divSpan.style.maxHeight = "0";
        if (isOn === true) {
            divSpan.style.transition = "max-height 0.2s ease";
            divSpan.style.maxHeight = "0";
            divWhiteBg.style.backgroundColor = "rgba(0,0,0,0)";
    ...
    // run the functions
    let i = false;
    document.querySelector("#Sdlds").addEventListener('click', function() {i = CollapseNavGatesToggle("#Sdlds", i)}, true);
